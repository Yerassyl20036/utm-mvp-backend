...
class User(Base):
    ...
    organization_id = Column(
        Integer,
        ForeignKey("organizations.id", name="fk_user_organization_id"),
        nullable=True,
    )
    ...
    assigned_drones_association = relationship(
        "UserDroneAssignment", back_populates="user", cascade="all, delete-orphan"
    )

    submitted_flight_plans = relationship(
        "FlightPlan",
        foreign_keys="[FlightPlan.user_id]",
