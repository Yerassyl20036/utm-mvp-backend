...
class Drone(Base):
    ...
    organization_id = Column(
        Integer,
        ForeignKey("organizations.id", name="fk_drone_organization_id"),
        nullable=True,
    )
    organization_owner = relationship(
        "Organization", foreign_keys=[organization_id], back_populates="drones"
    )

    # If owned by a solo pilot
    solo_owner_user_id = Column(
        Integer,
        ForeignKey("users.id", name="fk_drone_solo_owner_user_id"),
        nullable=True,
    )
    solo_owner_user = relationship(
        "User", foreign_keys=[solo_owner_user_id], back_populates="owned_drones"
    )

    current_status = Column(
        SQLAlchemyEnum(DroneStatus), default=DroneStatus.IDLE, nullable=False
    )
    last_telemetry_id = Column(
        Integer,
        ForeignKey(
            "telemetry_logs.id", name="fk_drone_last_telemetry_id", use_alter=True
        ),
        nullable=True,
    )  # use_alter for potential circular dep
    ...
    # Relationships
    assigned_users_association = relationship(
        "UserDroneAssignment", back_populates="drone", cascade="all, delete-orphan"
    )
