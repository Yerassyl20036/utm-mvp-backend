...
from app.db.base_class import \
    Base  # Base already has created_at, updated_at. Not strictly needed here unless desired.


class UserDroneAssignment(
    Base
):  # Inheriting from Base gives id, created_at, updated_at
    ...
    user_id = Column(
        Integer, ForeignKey("users.id", name="fk_assignment_user_id"), primary_key=True
    )
    drone_id = Column(
        Integer,
        ForeignKey("drones.id", name="fk_assignment_drone_id"),
        primary_key=True,
    )
    assigned_at = Column(
    ...
    def __repr__(self):
        return (
            f"<UserDroneAssignment(user_id={self.user_id}, drone_id={self.drone_id})>"
        )
